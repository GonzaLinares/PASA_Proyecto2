...
def plot_compare(resultsLMS, resultsRLS, P, S, xgen, soundgen):
    ...
    plt.xlabel('n')
    plt.ylabel('Square error')
    plt.semilogy(ran, J_lms, alpha=0.3, label='J LMS')
    plt.semilogy(ran, J_rls, alpha=0.3, label='J RLS')
    plt.legend()
    ...
    plt.xlabel('t [s]')
    plt.ylabel('modulo del error [dB]')
    plt.plot(ran, 20*np.log(np.abs(e_lms)), alpha=0.3, label='error LMS')
    plt.plot(ran, 20*np.log(np.abs(e_rls)), alpha=0.3, label='error RLS')
    plt.ylim([-100,80])
    plt.legend()
    ...
    plt.title('Comparativa entre módulo de W(z) y P(z)/S(z)')
    plt.semilogx(freqs, 20*np.log10(np.abs(s)), label='P(z)/S(z)')
    plt.semilogx(freqw_lms, 20*np.log10(np.abs(wps_lms)), label='$W_{LMS}(z)$', alpha = 0.3)
    plt.semilogx(freqw_rls, 20*np.log10(np.abs(wps_rls)), label='$W_{RLS}(z)$', alpha = 0.3)
    plt.xlabel('f [Hz]')
    plt.ylabel('Modulo [dB]')
    plt.xlim([20,20000])
    plt.legend()
